document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ключевые слова для определения бренда
    ...
    // Функция определения бренда по title
    const getBrandFromTitle = (title) => {
        ...
        return 'balenciaga'; // по умолчанию
    };
    ...
    function renderCards() {
        ...
        savedCards.forEach(card => {
            ...
            let updated = imgSrc;
            if (!(imgSrc.startsWith('./') || imgSrc.startsWith('http') || imgSrc.startsWith('/'))) {
                updated = `./${brand}/img/${imgSrc}`;
            }

                return `
                    <a href="${correctedHref}" class="swiper-slide">
                        <img src="${updated}" alt="${card.title}" />
                    </a>
                `;
            }).join('');
